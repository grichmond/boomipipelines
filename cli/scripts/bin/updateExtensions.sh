...
while IFS= read -r line
 do
        ...
        then
                ...
                echo "$line" | python ./bin/replaceText.py --original="$valueText" --replacement="$extensionValue" --outfile="$JSON_FILE"
        elif  [[ "$line" == *"valueCSV"* ]]
        then
                valueText=$(echo "$line" | sed -e 's/\"//g' -e 's/,//g' -e 's/^.*:\s\+//' -e 's/\s+.*$//g')
                echov $valueText
                ...
                csvFile=${csvFile^^}
                echo "$line" | python ./bin/replaceJSON.py ${!csvFile} $valueText $JSON_FILE
        else
                echo "$line" >> "${JSON_FILE}"
        fi
 done < "$TMP_JSON_FILE"
